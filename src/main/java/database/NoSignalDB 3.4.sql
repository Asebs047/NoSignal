...
delimiter //
	...
	begin
		insert into facturas (idusuario, idcarrito, fecha, subtotal, impuestos, total, estado, metodopago)
		values (p_idusuario, p_idcarrito, p_fecha, p_subtotal, p_impuestos, p_total, p_estado, p_metodopago);
...
delimiter //
	create procedure sp_actualizartotalcarrito(in p_idcarrito int)
	begin
		update carritoproductos c
		set total = (
...
-- Todo esto es para las facturas
-- Procedimiento para obtener el carrito activo de un usuario
-- obtener carrito activo
delimiter //
	create procedure sp_obtenercarritoactivo(in p_idusuario int)
	begin
		select * from carritoproductos 
		where idusuario = p_idusuario 
		and idcarrito not in (select idcarrito from facturas)
		limit 1;
	end //
delimiter ;

delimiter //
	create procedure sp_crearcarrito(in p_idusuario int)
	begin
		insert into carritoproductos (idusuario, total) values (p_idusuario, 0);
		select last_insert_id() as idcarrito;
	end //
delimiter ;

delimiter //
	create procedure sp_obtenerdetallescarrito(in p_idcarrito int)
	begin
		select d.*, p.nombre, p.precio, p.urlimagen 
		from detallescarritosproductos d
		join productos p on d.idproducto = p.idproducto
		where d.idcarrito = p_idcarrito and d.estado = 'sin pagar';
	end //
delimiter ;
...
delimiter //
	...
	begin
		...
		select count(*) into v_existe from detallescarritosproductos 
		where idcarrito = p_idcarrito and idproducto = p_idproducto and estado = 'sin pagar';
		
		if v_existe > 0 then
			-- actualizar cantidad si ya existe
			update detallescarritosproductos 
			set cantidad = cantidad + p_cantidad, 
				subtotal = (cantidad + p_cantidad) * v_precio
			where idcarrito = p_idcarrito and idproducto = p_idproducto and estado = 'sin pagar';
		else
			-- insertar nuevo detalle
			insert into detallescarritosproductos (idcarrito, idproducto, cantidad, subtotal, fechaagregado, estado)
			values (p_idcarrito, p_idproducto, p_cantidad, p_cantidad * v_precio, curdate(), 'sin pagar');
		end if;
		
		-- actualizar total del carrito
		call sp_actualizartotalcarrito(p_idcarrito);
	end //
delimiter ;
...
delimiter //
	...
	begin
		...
		-- obtener precio del producto y id del carrito
		select p.precio, d.idcarrito into v_precio, v_idcarrito
		from detallescarritosproductos d
		join productos p on d.idproducto = p.idproducto
		where d.iddetalle = p_iddetalle;
		
		update detallescarritosproductos 
		set cantidad = p_cantidad, 
			subtotal = p_cantidad * v_precio
		where iddetalle = p_iddetalle;
		
		call sp_actualizartotalcarrito(v_idcarrito);
	end //
delimiter ;

delimiter //
	create procedure sp_eliminarproductocarrito(in p_iddetalle int)
	begin
		declare v_idcarrito int;
		
		select idcarrito into v_idcarrito from detallescarritosproductos where iddetalle = p_iddetalle;
		
		delete from detallescarritosproductos where iddetalle = p_iddetalle;
...
delimiter //
	create procedure sp_vaciarcarrito(in p_idcarrito int)
	begin
		delete from detallescarritosproductos where idcarrito = p_idcarrito;
		update carritoproductos set total = 0 where idcarrito = p_idcarrito;
	end //
delimiter ;
...
-- Procedimiento para finalizar compra y hacer la factura xd
delimiter //
	...
	begin
		...
		insert into facturas (idusuario, idcarrito, fecha, subtotal, impuestos, total, estado, metodopago)
		values (p_idusuario, p_idcarrito, curdate(), v_subtotal, v_impuestos, v_total, 'completa', p_metodopago);
		...
		update productos p
		join detallescarritosproductos d on p.idproducto = d.idproducto
		set p.cantidad = p.cantidad - d.cantidad
		where d.idcarrito = p_idcarrito and d.estado = 'pagado';
...
call sp_agregarfactura(1, 1, '2023-11-10', 429.98, 51.60, 481.58, 'completa', 'tarjeta');
call sp_agregarfactura(2, 2, '2023-11-11', 599.99, 72.00, 671.99, 'completa', 'tarjeta');
call sp_agregarfactura(3, 3, '2023-11-12', 99.98, 12.00, 111.98, 'completa', 'efectivo');
call sp_agregarfactura(4, 4, '2023-11-13', 199.99, 24.00, 223.99, 'completa', 'transferencia');
call sp_agregarfactura(5, 5, '2023-11-14', 399.99, 48.00, 447.99, 'completa', 'tarjeta');
